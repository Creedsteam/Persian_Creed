...
local function pre_process(msg)
  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
      ...
      --save it to log file
      savelog(msg.to.id, name.." ["..msg.from.id.."] اسپم داد و اخراج شد ! ")
      -- incr it on redis
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." از همه ی گروه ها مسدود شد . بخاطر اسپم متعدد")
          local log_group = 1 --set log group caht id
          --send it to log group
          send_large_msg("chat#id"..log_group, "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." به کلی بن شد ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (بخاطر اسپم دادن)")
        end
      end
      kicktable[user] = true
      msg = nil
