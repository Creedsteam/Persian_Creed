...
local function check_member_autorealm(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'به خانه ی جدیدتان خوش آمدید!')
    end
  end
end
local function check_member_realm_add(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(realms)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'خانه اضافه شد')
    end
  end
end
...
local function realmadd(msg)
  ...
  if is_realm(msg) then
    return 'خانه از قبل اضافه شده بود'
  end
    receiver = get_receiver(msg)
...
local function run(msg, matches)
  ...
  if matches[1] == 'اضافه شود' and not matches[2] then
    if is_realm(msg) then
       return 'خطا : اینجا به عنوان گروه خانه معرفی شده است'
    end
    print("گروهه "..msg.to.print_name.."("..msg.to.id..") اضافه شد")
    return modadd(msg)
  end
   if matches[1] == 'اصافه شود' and matches[2] == 'به عنوان خانه' then
    if is_group(msg) then
       return 'خطا : اینجا به عنوان گروه معمولی معرفی شده است'
    end
    print("گروهه "..msg.to.print_name.."("..msg.to.id..") به عنوان گروه خانه معرفی شد")
    return realmadd(msg)
  end
  if matches[1] == 'حذف شود' and not matches[2] then
    print("گروهه "..msg.to.print_name.."("..msg.to.id..") حذف شد")
    return modrem(msg)
  end
  if matches[1] == 'حذف شود' and matches[2] == 'از گروه خانه' then
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'نصب مدیر' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
    ...
    if matches[1] == 'صلب مدیر' and not matches[2] then
      if not is_owner(msg) then
    ...
    if matches[1] == 'صلب مدیر' and matches[2] then
      if not is_momod(msg) then
        return
      end
      if not is_owner(msg) then
   ...
   if matches[1] == 'قفل' then
    ...
    if matches[1] == 'باز کردن' or matches[2] = 'آزاد کردن' then
      local target = msg.to.id
      if matches[2] == 'همه' then
      	if not is_momod(msg) then
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'خراب کردن' and matches[2] == 'گروه' then
      ...
      if not is_realm(msg) then
          ...
          return 'این یک گروه خانه هست'
      end
   end
    if matches[1] == 'خراب کردن' and matches[2] == 'خانه' then
     if not is_admin(msg) then
         return nil
     end
     if not is_group(msg) then
...
return {
  ...
  patterns = {
  "^(اضافه شود)$",
  "^(اضافه شود) (به عنوان خانه)$",
  "^(حذف شود)$",
  "^(حذف شود) (از گروه خانه)$",
  "^(قوانین)$",
  "^(توضیحات)$",
  "^(نصب اسم) (.*)$",
  "^(نصب عکس)$",
